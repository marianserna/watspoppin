...
# Scopes
  ...
  def access_token
      send("#{provider}_refresh_token!", super) if expired? #send calls to _refresh_token method dynamically
...
# Facebook Methods
  ...
  def facebook_refresh_token!(token)
    new_token_info = Koala::Facebook::Omniauth.new.exchange_access_token_info(token)  #talks to database and grab token and talk to koala and exchange the old token with new one from facebook
    udpate(
